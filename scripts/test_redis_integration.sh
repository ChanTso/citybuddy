...
commerce_mount="$(docker inspect --format '{{range .Mounts}}{{if eq .Destination "/data"}}{{.Name}}:{{.Destination}}{{end}}{{end}}' "$commerce_container")"
support_mount="$(docker inspect --format '{{range .Mounts}}{{if eq .Destination "/data"}}{{.Name}}:{{.Destination}}{{end}}{{end}}' "$support_container")"
