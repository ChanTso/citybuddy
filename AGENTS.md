...
8. Stop continuous execution and report the reason when repository state matches zero or multiple states above; a slice becomes `BLOCKED`; evidence conflicts with a frozen contract; `main`, branch, or pull-request state is ambiguous or contradictory; a required check, review, merge, credential, or permission fails; or continuation requires new user authority.
