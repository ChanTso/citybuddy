...
6. If review, repository policy, mergeability, or a check failure outside the recoverable same-slice CI path below blocks completion, stop and report the blocker; do not bypass it.
...
9. One recovery cycle is one evidence-based correction followed by the required reruns. Perform at most two consecutive recovery cycles for the same failure class; a full pass of all required checks resets that limit. Do not evade the limit through cosmetic changes, retries without a correction, test weakening, skipped checks, or a new branch or pull request.
10. Stop continuous execution and report the reason when repository state matches zero or multiple states above; a slice becomes `BLOCKED`; evidence conflicts with a frozen contract; `main`, branch, or pull-request state is ambiguous or contradictory; a CI failure is unrelated to the current slice, has ambiguous evidence, or persists after two recovery cycles; a correction requires changed contracts, route, scope, dependencies, secrets, permissions, or user authority; review, merge, credentials, repository policy, CI infrastructure, or mergeability blocks progress; or continuation would bypass a required check.
