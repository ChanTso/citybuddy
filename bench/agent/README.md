...
The commerce container therefore received four of Docker's eight CPUs. Four is a fixed local
fixture choice, not a value derived from production sizing or from these measurements. Historical
setup JSON records the eight-CPU Docker allocation but not commerce's live `HostConfig.NanoCpus`;
the four-CPU boundary is recoverable from the measured runner, not self-proved by those artifacts.
New runs record both the single requested value and live NanoCpus and fail publication on drift;
that does not retroactively strengthen the historical bundles.
...
that establishes CPU headroom, not spare database capacity in every dimension. The control
experiment says raising the connection limit is not throughput headroom: it did not raise what the
path served, but converted fast rejections into long queues. Read
...
**Nothing in the agent bounds its own concurrency on any path.** Chat first exposes a configured
MySQL connection-rejection boundary, but the 1,000-connection control did not increase its clean
served rate, so that boundary explains overload behavior rather than the throughput cause.
