...
component and full-path throughput. Retained raw and derived artifacts are in `results/`;
their measurement and evidence boundaries are stated below.
...
written from the hard-coded value passed to Docker; it was not a live observation of
`HostConfig.NanoCpus`. That field records the requested fixture configuration, not an inspected
runtime limit, and does not retroactively prove a live resource allocation.
...
The sampled peaks transcribed in `results/peak_cpu.txt` were 145% for commerce against its requested
400% allowance, 61% for MySQL, and 60% for k6. The raw CPU series also sampled Redis. These readings
describe the captured samples; they do not rule out CPU saturation between samples or establish
headroom in an unsampled resource. The topology comparison below is the evidence that connects
the collapse to same-row serialization.

### 4. Where the serialization is

In the historical MySQL-first implementation measured below,
`SeckillReservationService.reserveIntent` opened its transaction with:

```sql
SELECT ... FROM seckill_activity WHERE activity_id = ? FOR UPDATE
```

and held that single row through the idempotency lookup, the reservation insert, and a re-read
until commit. Every reservation for one activity passed through that row. The Redis Lua quota
decision happened *after* this transaction, making the activity-row lock the serialization candidate
that the 32-activity counterfactual below tests; the CPU samples alone do not establish it.
The Redis-first path described above is a later implementation, not this historical sequence.
...
`results/innodb_deadlock.txt` preserves these details. The separate payment-callback graph in
[`mock_payment_callback_deadlock.txt`](results/mock_payment_callback_deadlock.txt) belongs to a
different lock path and is not evidence for this seckill cycle:
