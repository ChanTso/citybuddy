...
| [Seckill reservation contention](bench/README.md#finding-and-fix-the-serializing-lock-was-also-suppressing-a-deadlock) | The pre-fix 32-activity ladder returned 6,202 HTTP 500s in 23,254 requests; its retained log grep contains 12,404 matching text lines, not an exact deadlock-event count. The fixed spread run returned 0/23,256 failed requests and its retained grep found 0 matches. At the 800 req/s offered target, 12,003 admissions completed; the legacy first-to-last-completion-span density was 799.9/s, with p99 39.4 ms and 0 drops. | One MacBook Pro M4 (10 cores, 24 GB), Docker Desktop (8 CPUs, 14 GB), Commerce limited to 4 CPUs, authenticated requests across Redis Lua and real MySQL transactions; 32 activities, a 15 s fixed-arrival-rate step, setup excluded. This is a local workload result, not a capacity claim. |
