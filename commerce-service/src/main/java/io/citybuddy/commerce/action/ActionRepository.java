...
public class ActionRepository {
  ...
  public Optional<PendingActionRecord> findPendingByTurnForUpdate(
    ...
    return queryPending(
        "SELECT "
            ...
            + "AND turn_id = ? LIMIT 2 FOR UPDATE",
        user,
        session,
        turnId);
  }
  ...
  public Optional<PendingActionRecord> findPendingByIdForUpdate(String pendingActionId) {
    return queryPending(
        "SELECT "
            + pendingColumns()
            + " FROM pending_action WHERE pending_action_id = ? LIMIT 2 FOR UPDATE",
        pendingActionId);
  }
  ...
  public Optional<ActionReceiptRecord> findReceiptByPending(String pendingActionId) {
    return queryReceipt(
        "SELECT " + receiptColumns() + " FROM action_receipt WHERE pending_action_id = ? LIMIT 2",
        pendingActionId);
  }
  ...
  private static String pendingColumns() {
    return "pending_action_id, action_idempotency_key, pending_hash, action_type, argument_hash, "
        + "user_subject, support_session_id, trace_id, turn_id, required_scope, sandbox_id, order_id, "
