...
import { SseParser, UnsupportedReceiptError } from './sse';
...
describe('bounded SSE parser', () => {
  it('handles fragmented chunks, multiple frames, Unicode, LF and CRLF', () => {
    ...
    expect(parser.finish()).toEqual({ reply: '你好 ', outcome: 'completed' });
  });
  ...
  it('validates reserved action_receipt then stops without returning its payload', () => {
    const parser = new SseParser();
    const frame = `event: action_receipt\ndata: {"sequence":1,"receiptId":"${UUID}","status":"SUCCEEDED"}\n\n`;
    expect(() => parser.push(encode(frame))).toThrow(UnsupportedReceiptError);
  });
