...
| `web` or evaluator → `agent-service` | `POST /api/chat/stream` | Direct user JWT | Same direct-user and support-session rules as `/api/chat` | SSE emits only `token`, `action_receipt`, `done`, and `error`; `token` is non-authoritative prose and clients derive action status only from `action_receipt` | Same identity/session/sandbox failures; no raw tool/retrieval output | `CB-082`; receipt/turn integration in `CB-121` |
...
| Mock payment component → `commerce-service` | `POST /internal/mock-payments/callback` | Separate internal callback credential/signature | Callback idempotency and payment/order correlation; exact sandbox binding when applicable | Applies one legal transition; duplicate returns existing result | Invalid credential, unknown correlation, sandbox mismatch/inactivity, or illegal transition rejects and audits | `CB-070`; evaluation sandbox extension in `CB-105` |
...
The shared definition distinguishes exact/invariant-backed participating columns from columns whose original value has no independent durable anchor. A complete-diff review proved that merely listing `mock_payment_attempt.succeeded_at` without reading and asserting it still permitted `409/200/200`; the attempt event time, canonical attempt intent, and evaluation-only zero-refund accumulator are therefore explicit callback/view assertions and independent fault cells, and compound state/version/time mutations do not count as per-column evidence. The zero-refund rule is not part of production committed-payment replay: a legal later partial or full refund changes that accumulator without invalidating the immutable original callback result. Three internal-only residual dispositions are recorded in executable metadata under the final cap: the start-command idempotency key has database uniqueness and bounded format but no post-creation external value anchor; `evaluation_owner_handle` is reset provenance while committed replay is anchored to effective `user_subject`; and the database-generated ledger `movement_id` has primary-key identity but no second content anchor. Valid-value substitution of those three fields is not claimed detectable by the internal evaluation view. This residual record does not weaken their production idempotency, refund, or transaction constraints, and any real business-path inconsistency remains blocking.
...
**Resolved Level 3 contract amendment — 2026-07-26 (CB-120 payment/refund contention-safety closure):** CB-120's shared committed-payment resolver exposed existing direct-refund and refund-lifecycle transactions to payment callback lock competition. The owner rejected a transaction-wide or cross-service acyclic lock protocol for this slice and approved a narrow correction to the active slice's frozen failure contract, not a route decision. The refund state machine, ownership and idempotency rules, normal success/replay semantics, payment/refund truth ownership, PendingAction/ActionReceipt atomicity, route dependencies, schema and runtime grants remain unchanged. Every affected top-level payment or refund transaction must use a shared neutral executor that sets a validated physical `innodb_lock_wait_timeout` on the transaction's actual JDBC session and restores the prior session value before pool reuse; payment and refund retain separate business entry inventories and configuration. Only a cause-chain `SQLException` with MySQL vendor code 1205 or 1213 enters contention recovery, and recovery begins only after the original transaction callback has failed and rolled back. Each re-observation uses a new bounded transaction. In-transaction Action refund helpers propagate contention so the owning Action transaction rolls back and performs its existing outer recovery; they never continue in the failed transaction or create a nested recovery transaction.

For direct refund, a bounded locking observation of complete owner-visible refund and payment truth may return the durable replay or confirm absence and authorize at most one final mutation. If truth remains unobservable, the public boundary returns one bounded closed-schema 429 `INDETERMINATE`; dependency resource failure remains 503, established durable damage or intent conflict remains 409, and established concealment retains its existing 404 family. Internal refund lifecycle and reconciliation entries return a typed retryable/indeterminate outcome to their owning boundary and never turn observation failure into `CONTRADICTION` or another terminal business decision. The amendment becomes effective only with real MySQL evidence for physical 1205/1213 classification, complete rollback, bounded completion, connection-pool session-policy restoration, no unclassified 500, no partial refund/ledger/Outbox effects, no erroneous PendingAction consumption or duplicate ActionReceipt, and exact OpenAPI/public-status parity. Residual risk is precise: the payment/refund subgraph may still incur avoidable lock competition under high concurrency, but every affected transaction is physically bounded and fully rolled back; after bounded durable-truth re-observation, its owner can return only an existing durable result or an explicit retryable/indeterminate outcome (HTTP 429 only at a public HTTP boundary). It cannot produce a partial commit, false business contradiction, false dependency-unavailable result, or unclassified 500. A global acyclic lock protocol is deferred high-concurrency availability and contention-rate optimization, not ignored review evidence.

Current Level 3 conflicts: **none unresolved as of 2026-07-26**.
