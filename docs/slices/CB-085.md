...
| Field | Value |
|---|---|
| Branch | `codex/cb-085-python-rocketmq-spike` |
| PR | [#15](https://github.com/ChanTso/citybuddy/pull/15) |
| Commits | `73f0c8a` |
| Tests | `uv run pytest tests/test_rocketmq_spike.py` (5 passed); non-writable-`HOME` isolated `uv run pytest tests/test_rocketmq_spike.py` (5 passed); `uv run pytest` (24 passed); `uv run ruff check .`; `uv run mypy agent-service/src agent-service/tests knowledge-indexer/src knowledge-indexer/tests scripts tests`; `bash -n scripts/test_knowledge_indexer_rocketmq_spike.sh`; `make test-knowledge-indexer-rocketmq-spike`; `make ci` |
| Notes | The pinned RocketMQ 5.5.0 Broker/Proxy, Python 3.11.15, `rocketmq-python-client` 5.1.1, and Elasticsearch 8.19.8 combination is viable for the later disposable knowledge-indexer boundary when the consumer runs on the same Docker network as the Proxy. SimpleConsumer was selected because explicit acknowledgement and invisible-duration extension were executed successfully; PushConsumer retry and long-processing behavior were also exercised but it was not selected because its public constructor does not expose the required invisible-duration control. The spike additionally proved filtering, bounded redelivery with duplicate-safe handling, source-version rejection, idempotent tombstones, rebuild validation, and atomic alias handoff. Initial independent review found four blockers (lockfile enforcement, PushConsumer long-processing evidence, exact runtime-version evidence, and SDK import side effects); all were corrected, and a fresh independent read-only review returned `NO BLOCKER`. This is a viability result, not a production consumer or compatibility guarantee. |
