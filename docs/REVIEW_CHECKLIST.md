...
### Measurement manifest truth and committed-candidate policy scans

- Reconcile every environment and resource statement in a measurement manifest against the actual
  owners used by the runner: host process versus container, network attachment, port allocation,
  child-process ownership, resource limits, and cleanup labels must match observed runtime facts.
  A reconstructable result with inaccurate topology metadata is invalid evidence and must be
  regenerated by the corrected producer rather than hand-edited.
- Bind the formal bundle to a clean committed candidate and run repository policy scanners against
  the exact reachable commit history before spending the formal evidence run. A staged-diff or
  working-tree secret hook does not prove the committed candidate will pass the repository-history
  scanner. Resolve a false positive without allowlists, excludes, rule weakening, or leaving the
  offending unpublished commit reachable; then repeat the complete evidence lifecycle when the
  runner or bound candidate changes.
