...
- 根因：回放路径在解析完整已提交真值之前进入了可变 sandbox 存活路径；首轮修复又在独立事务中只按 callback key/event 预读，删除 callback 行或让预读发生在并发提交前，仍会把已提交结果误判为不存在并落入 liveness fence。`DataAccessException -> 503` 同时覆盖连接不可用、`CannotAcquireLockException` 与 `DuplicateKeyException`，也使瞬时竞争和约束冲突无法按根因收敛。闭包最初只枚举“哪些面”，没有把每一面的基数 `0`/`1`/`>=2` 明确列为独立有界维度；测试只删改预期行，未注入额外行。测试编排则把 MySQL 容器恢复或管理连接可用误当作应用连接池已恢复，并一度用会改变 Docker 动态宿主端口的 stop/start 制造了无法重连的假恢复窗口。
- 解决：callback 在同一加锁事务中先以稳定 correlation 反向枚举 callback、attempt、order、ledger、audit 五个已提交面，并用签名请求的 order/event/context 在一个面缺失时保留其他面；任一已提交面存在便先做完整规范意图与五面对账，相同意图返回既有结果，冲突、缺面或数据损坏固定 409，只有五面均无已提交结果才进入 sandbox liveness fence。闭包升级为“5 个面 × 基数 `0`/`1`/`>=2` × 存在行的全部内容列”；sandbox 只产生待查稳定键，外层按该键枚举全表行后才断言 sandbox/type/status，避免损坏 sibling 被作用域谓词排除。repository 用专用完整性异常报告 attempt/order/callback 多行，callback 操作边界按根因翻译为 409，ledger 与 audit 从稳定身份无类型过滤地统计完整行集并拒绝多行。真实矩阵除删行、移出稳定键、逐列篡改和两两组合外，还分别为五面注入一条跨 sandbox 第二行；约束保护的 attempt/order/ledger 只在隔离测试库中暂时移除约束、取证后立即恢复。1213 deadlock、1205 lock timeout 与 duplicate-key 竞争都在 service 内重新解析提交真值，收敛为既有结果或 409。HTTP 503 映射收窄到连接/资源不可用和事务无法建立。真实集成以 MySQL `offline_mode` 在不改变 endpoint 的情况下断开运行连接，先证明断连固定 503，再通过触达数据库的认证 `/api/eval/state` 200 门确认 Hikari 重连；门后五次不同 callback key 回放均固定 409，相同意图回放返回既有结果，已死亡 sandbox 的已提交回放仍返回 200 且无二次写入。
- 结论：已提交的幂等结果必须先由完整 durable truth 解析，不能被之后变化的 admission/liveness 覆盖；异常到 HTTP 的映射必须按根因和子类精确分类，宽泛数据库超类会把锁竞争或业务冲突误渲染为不可用，完整性损坏也不得以通用异常逃逸为 500。损坏矩阵必须显式声明轴，而不是只列作者想到的实例：面、基数和内容都是有界轴，应构成闭包；若出现新轴，先由所有者判断能否机械枚举，无界值域按 CB-112 先例登记残余风险。容器健康不等于应用就绪：依赖恢复后必须以真实触达该依赖的应用端点为连接池设门，否则正确的 503 会被误判为产品缺陷；动态 runtime-owned endpoint 也不能在仍存活的依赖方背后静默变化。
