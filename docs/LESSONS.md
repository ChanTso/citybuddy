...
- 现象：首轮真实 MySQL migration 因 feedback 外键引用的 support-turn 关联列没有显式复合唯一键而失败；修正后，反馈事务先后尝试锁定 `support_session` 和 `support_feedback`，又被最小权限 `agent_app` 以 1142 拒绝。真实身份集成还发现 SSE 路由把 owner `HTTPException` 吞成 HTTP 200 的公共 error 事件，而旧 crash-window fixture 硬编码的 event sequence 与新增 SSE turn 发生碰撞。独立复核最终又证明原 action-success 英文正则过窄：`Your cancellation is complete.` 在没有 ActionReceipt 时仍能发出 token 和 done。
- 证据链接：[slice PR #33](https://github.com/ChanTso/citybuddy/pull/33)、[implementation and integration recovery `63103c8`](https://github.com/ChanTso/citybuddy/commit/63103c8)、[action-claim review recovery `e07d867`](https://github.com/ChanTso/citybuddy/commit/e07d867)
- 根因：MySQL 外键只认可被引用列的显式索引唯一性，不能从相邻单列约束推断业务复合关联；锁定读取在 InnoDB 中需要目标表的 `UPDATE` 权限，即使代码不执行更新。路由异常边界又把“已验证的授权拒绝”与“流内私有运行失败”合并处理，历史集成 fixture 也把会随新 turn 增长的序号写成常量。输出过滤则把少数完整英文短语当成安全边界，没有先做 Unicode/不可见格式字符归一化，也没有把敏感动作词与完成状态词作为独立 fail-closed 信号组合。
- 解决：为 support turn 增加明确的 trace/session/user 复合唯一键；反馈同一会话的并发请求只在已有写权限的 `support_conversation` 聚合根上 `FOR UPDATE` 串行化，再普通读取不可变 session/turn 和 append-only feedback truth，因此无需扩大 feedback/session 权限。SSE 保留 `HTTPException` 的真实 401/403/409 等状态，只把未预期的私有执行失败投影为固定公共 error；旧 crash-window sequence 从数据库当前最大值推导。action guard 先执行 NFKC/casefold 并删除不可见格式字符，再组合已知英文/中文敏感动作与完成词拒绝无 receipt 的成功断言，同时保留非成功的退款说明、pending order 和 payment option 文本；单元 fake 与真实 fake-provider 都改用评审发现的 cancellation bypass 取证。四个并发同意图反馈真实收敛到一个 row，完整身份集成与 `make ci` 随后通过。
- 结论：外键、锁和最小权限必须在真实数据库上作为一个整体设计：串行化点应选择已有写权限的业务聚合根，不能为方便锁读而扩大 append-only 表权限。HTTP 层的授权拒绝与已开始流后的私有错误也必须分层，历史测试中的顺序值应从权威状态推导。自然语言安全边界不能只枚举几条“看起来典型”的整句；至少要先归一化输入、分离动作与完成信号、加入真实绕过语料，并在没有权威 receipt 时偏向拒绝。
