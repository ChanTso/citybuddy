...
- 现象：CB-113 final-head CI 在已经提交成功的 payment callback 使用不同 callback key 回放时，期望确定性 409，却得到 `PAYMENT_SANDBOX_NOT_ACTIVE` 403。初版维护修复让已提交回放绕过不活跃存活判定后，受控竞争又把锁等待异常落入控制器的宽泛数据库异常兜底而返回 503。真实断连恢复后的首个请求也曾得到 503；后者最终证明是 Hikari 尚未重连时的真实不可用，而非分类回归。
- 证据链接：[maintenance PR #50](https://github.com/ChanTso/citybuddy/pull/50)
- 根因：回放路径在解析完整已提交真值之前进入了可变 sandbox 存活路径；首轮修复又在独立事务中只按 callback key/event 预读，删除 callback 行或让预读发生在并发提交前，仍会把已提交结果误判为不存在并落入 liveness fence。`DataAccessException -> 503` 同时覆盖连接不可用、`CannotAcquireLockException` 与 `DuplicateKeyException`，也使瞬时竞争和约束冲突无法按根因收敛。测试编排则把 MySQL 容器恢复或管理连接可用误当作应用连接池已恢复，并一度用会改变 Docker 动态宿主端口的 stop/start 制造了无法重连的假恢复窗口。
- 解决：callback 在同一加锁事务中先以稳定 correlation 反向枚举 callback、attempt、order、ledger、audit 五个已提交面，并用签名请求的 order/event/context 在一个面缺失时保留其他面；任一已提交面存在便先做完整规范意图与五面对账，相同意图返回既有结果，冲突、缺面或数据损坏固定 409，只有五面均无已提交结果才进入 sandbox liveness fence。真实单面与两两一致性故障注入以及“回放等待 callback 提交、随后 completion 先完成”的确定性窗口验证该顺序。1213 deadlock、1205 lock timeout 与 duplicate-key 竞争都在 service 内重新解析提交真值，收敛为既有结果或 409。HTTP 503 映射收窄到连接/资源不可用和事务无法建立。真实集成以 MySQL `offline_mode` 在不改变 endpoint 的情况下断开运行连接，先证明断连固定 503，再通过触达数据库的认证 `/api/eval/state` 200 门确认 Hikari 重连；门后五次不同 callback key 回放均固定 409，相同意图回放返回既有结果，已死亡 sandbox 的已提交回放仍返回 200 且无二次写入。
- 结论：已提交的幂等结果必须先由完整 durable truth 解析，不能被之后变化的 admission/liveness 覆盖；异常到 HTTP 的映射必须按根因和子类精确分类，宽泛数据库超类会把锁竞争或业务冲突误渲染为不可用。容器健康不等于应用就绪：依赖恢复后必须以真实触达该依赖的应用端点为连接池设门，否则正确的 503 会被误判为产品缺陷；动态 runtime-owned endpoint 也不能在仍存活的依赖方背后静默变化。
