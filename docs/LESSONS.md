...
- 结论：已提交的幂等结果必须先由完整 durable truth 解析，不能被之后变化的 admission/liveness 覆盖；异常到 HTTP 的映射必须按根因和子类精确分类，宽泛数据库超类会把锁竞争或业务冲突误渲染为不可用，完整性损坏也不得以通用异常逃逸为 500。损坏矩阵必须显式声明轴，而不是只列作者想到的实例：面、基数和内容都是有界轴，应构成闭包；若出现新轴，先由所有者判断能否机械枚举，无界值域按 CB-112 先例登记残余风险。同一领域概念若在多条路径各自定义必然漂移；唯一共享定义配合跨路径分类一致性断言，能比继续追加实例更早捕获这种分裂。容器健康不等于应用就绪：依赖恢复后必须以真实触达该依赖的应用端点为连接池设门，否则正确的 503 会被误判为产品缺陷；动态 runtime-owned endpoint 也不能在仍存活的依赖方背后静默变化。
