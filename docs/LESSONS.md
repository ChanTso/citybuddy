...
- 现象：最初把 Elasticsearch 已成功但 Redis finalize 失败的窗口直接留在旧 ready 状态，使 Agent 仍可能命中旧答案；改用独立 pending fence 后，`volatile-lfu` 又可单独淘汰 fence 并重新暴露旧 ready。把状态合并为 source-scoped preparation 后，首轮只修复 fence 缺失、仍把存在但损坏的 preparation 映射为 `KnowledgeSyncConflict` 并永久 ACK，且语义租约使用 `PERSIST`，没有物理回收机制。随后手写的 14 格状态 × 阶段矩阵全部只构造 `ready=0` preparation，遗漏同一生产 Lua 会读取的 `ready=1` 持久态；把合法 ready v3 的 `source_version` 单独损坏为格式仍合法的 v4 后，合法 v4 publication 会被 Redis 误判为业务冲突并在到达 Elasticsearch 前永久 ACK。第一版“机械闭包”又用正则只枚举 `HGET state`，把同一 Lua 读取的 versioned answer 12 个字段整体排除；18/18 返回标签则让 `ready_state_invalid_projection` 从其他 state 故障借到覆盖。改用活 Redis 枚举后，独立复核仍发现 FINALIZE 只拒绝 `PTTL <= 0`，而 PREPARE 已拒绝 preparation TTL 超上界或未超过语义 lease；把 TTL 改成仍为正但过长会在 ES 已成功窗口被直接改写并首投 ACK。
- 证据链接：[slice PR #48](https://github.com/ChanTso/citybuddy/pull/48)
- 根因：跨存储投影先后把可淘汰协调键当作唯一 fence、把语义租约当作物理生命周期，并让低层 Redis 对自身矛盾作终态业务裁决。人工矩阵的状态轴来自作者已想到的实例；改成源码正则后，正则选择的 operand、语法和返回标签粒度又成为新的隐式范围限定。它与此前 SQL 谓词正则被函数包裹绕过是第二次同型失败：解析生产代码文本的闭包装置无法证明自身选定作用域之外没有责任面。
- 解决：source 状态在权威写前原子切换为 fail-closed preparation，租约由 Redis `TIME` 取得并配套 121 秒物理 TTL（60 秒语义租约、60 秒固定安全余量和严格大于边界的 1 秒余度）。owner-local 矛盾只返回 authority-required；Worker 先让 Elasticsearch 判定，同版本真实冲突只有 Elasticsearch 可以终态 ACK，合法事件则以已确认权威结果重建 Redis、先 RETRY 再重放收敛。PREPARE 与 FINALIZE 现在复用同一 preparation-state guard，统一检查字段形状、TTL 上界、剩余 lease 和严格安全余量。矩阵不再解析 Lua 文本：分别用生产转换形成 `missing`、`ready0`、`ready1-live`、`ready1-tombstone`，再从真实 Redis 以 `SCAN`、`TYPE`、`PTTL`、`HGETALL` 枚举全部键、字段与键级属性；每格注入前重新枚举并与发现结果相等，新增字段或键面自动进入责任范围。最终 246 个真实格覆盖 45 个活字段目标和两个管线阶段，观察并精确匹配注册表中的 115 个 phase-scoped 判别码、360 次 RETRY，并逐格核对 source state、当前 versioned answer/应有缺席和 Elasticsearch 文档。TTL 正数超上界、不超过 lease、侵蚀安全余量三条具体设计评审回归均被两个共享 guard 拒绝。原始合法格式版本矛盾继续证明首投 RETRY、ES 推进、Redis 修复，重投才 ACK。
- 结论：修复缺失态而遗漏损坏态，是同一状态空间里的同胞实例；语义租约必须有配套物理生命周期。人工列举会遗漏作者未想到的状态，而解析生产代码文本的闭包装置会把自身作用域变成新的藏身处；闭包必须从活数据、schema 等被保护对象的地面真值枚举。聚合结果标签同时破坏线上归因与覆盖验证，每条已实现控制路径必须有判别码。低层真值不得独立确证业务冲突：owner-local 自相矛盾是完整性故障，终态只能由更高真值给出。可机械闭合与不可机械闭合的边界必须由所有者显式划定并登记残余风险：覆盖装置能证明已实现路径被执行，却永远发现不了从未实现的语义 guard；后者属于针对性设计评审。PREPARE/FINALIZE guard 不对称是这一残余风险的具体形态，本次以共享 guard 消除，而属性值域的穷尽分区不再被声称为矩阵完备性。时间相关故障注入必须落在目标分区内部并留出明确余量；把 TTL 设在随时间移动的等式边界，会因时钟推进与取整偶发落入相邻判别码，形成覆盖假阴性。
...
- 现象：CB-113 final-head CI 在已经提交成功的 payment callback 使用不同 callback key 回放时，期望确定性 409，却得到 `PAYMENT_SANDBOX_NOT_ACTIVE` 403。初版维护修复让已提交回放绕过不活跃存活判定后，受控竞争又把锁等待异常落入控制器的宽泛数据库异常兜底而返回 503。真实断连恢复后的首个请求也曾得到 503；后者最终证明是 Hikari 尚未重连时的真实不可用，而非分类回归。五面闭包落地后，独立复核再插入第二条相同 correlation 的 callback 行；普通索引允许该损坏形态，repository 的多行保护抛出通用 `IllegalStateException`，最终逃逸为 500。基数轴补齐后，复核又在同一 `order` 面插入与标准订单同 `order_id` 的秒杀订单：callback 路径跨 `standard_order`/`seckill_order` 判 409，evaluation state/audit 却因只读标准订单返回 200，形成 `409/200/200` 分类分裂。
- 证据链接：[maintenance PR #50](https://github.com/ChanTso/citybuddy/pull/50)
- 根因：回放路径在解析完整已提交真值之前进入了可变 sandbox 存活路径；首轮修复又在独立事务中只按 callback key/event 预读，删除 callback 行或让预读发生在并发提交前，仍会把已提交结果误判为不存在并落入 liveness fence。`DataAccessException -> 503` 同时覆盖连接不可用、`CannotAcquireLockException` 与 `DuplicateKeyException`，也使瞬时竞争和约束冲突无法按根因收敛。闭包最初只枚举“哪些面”，没有把每一面的基数 `0`/`1`/`>=2` 明确列为独立有界维度；测试只删改预期行，未注入额外行。更深处，同一个领域概念“order 面”在 callback 与 evaluation view 两条代码路径分别定义，物理表集合随实现演进发生漂移；audit 基数查询又曾把 session、trace、operation 分别用 `OR` 定位，可能把同 support session 的合法另一 operation 当成重复。测试编排则把 MySQL 容器恢复或管理连接可用误当作应用连接池已恢复，并一度用会改变 Docker 动态宿主端口的 stop/start 制造了无法重连的假恢复窗口。
- 解决：callback 在同一加锁事务中先以稳定 correlation 反向枚举 callback、attempt、order、ledger、audit 五个已提交面，并用签名请求的 order/event/context 在一个面缺失时保留其他面；任一已提交面存在便先做完整规范意图与五面对账，相同意图返回既有结果，冲突、缺面或数据损坏固定 409，只有五面均无已提交结果才进入 sandbox liveness fence。闭包升级为“5 个面 × 基数 `0`/`1`/`>=2` × 存在行的全部内容列”；sandbox 只产生待查稳定键，外层按该键枚举全表行后才断言 sandbox/type/status，避免损坏 sibling 被作用域谓词排除。五个面各自的物理表、稳定键和参与列收敛到唯一共享定义；callback resolver 与 evaluation view 都由它派生，order 面明确横跨标准/秒杀两表。每个与当前 callback 稳定键关联的真实损坏格同时驱动 signed callback、state、audit 并机械要求 `409:409:409`；无关联孤儿只让全表 state/audit totality 失败，不污染精确 callback replay。新增 cross-type 同键 sibling 回归。audit 基数只按 callback entity id 或完整的 sandbox/session/trace/operation 元组定位，并用同 session 多 operation 正向夹具证明合法共存。repository 用专用完整性异常报告 attempt/order/callback 多行，callback 操作边界按根因翻译为 409，ledger 与 audit 从稳定身份无类型过滤地统计完整行集并拒绝多行。真实矩阵除删行、移出稳定键、逐列篡改和两两组合外，还分别为五面注入一条跨 sandbox 第二行；约束保护的 attempt/order/ledger 只在隔离测试库中暂时移除约束、取证后立即恢复。完整 diff 复核随后发现共享元数据声明了 attempt `succeeded_at`，但 view 与矩阵没有消费它；修复把 event time、canonical attempt intent、refund accumulator 以及拆开的 state/version/time 纳入独立三路径断言，并将没有第二真值锚的 start key、fixture owner provenance、generated movement id 逐列登记为 owner-capped 内部残余，撤销“声明即覆盖”的假结论。三路径扩展使套件跨过原五分钟签名与 sandbox 窗口后，夹具改为逐格用当前时间重签同一规范意图，并把长跑 happy-path sandbox 明确延长到 1200 秒；专门的短期过期夹具不变。1213 deadlock、1205 lock timeout 与 duplicate-key 竞争都在 service 内重新解析提交真值，收敛为既有结果或 409。HTTP 503 映射收窄到连接/资源不可用和事务无法建立。真实集成以 MySQL `offline_mode` 在不改变 endpoint 的情况下断开运行连接，先证明断连固定 503，再通过触达数据库的认证 `/api/eval/state` 200 门确认 Hikari 重连；门后五次不同 callback key 回放均固定 409，相同意图回放返回既有结果，已死亡 sandbox 的已提交回放仍返回 200 且无二次写入。
- 结论：已提交的幂等结果必须先由完整 durable truth 解析，不能被之后变化的 admission/liveness 覆盖；异常到 HTTP 的映射必须按根因和子类精确分类，宽泛数据库超类会把锁竞争或业务冲突误渲染为不可用，完整性损坏也不得以通用异常逃逸为 500。损坏矩阵必须显式声明轴，而不是只列作者想到的实例：面、基数和内容都是有界轴，应构成闭包；若出现新轴，先由所有者判断能否机械枚举，无界值域按 CB-112 先例登记残余风险。同一领域概念若在多条路径各自定义必然漂移；唯一共享定义配合跨路径分类一致性断言，能比继续追加实例更早捕获这种分裂。内部表面不变式进入生产共用校验器前，必须枚举所有调用者及后续合法生命周期；合法退款会改变累计退款额，但不能使历史支付 callback 的精确回放失效。容器健康不等于应用就绪：依赖恢复后必须以真实触达该依赖的应用端点为连接池设门，否则正确的 503 会被误判为产品缺陷；动态 runtime-owned endpoint 也不能在仍存活的依赖方背后静默变化。
- 补充复核证据：共享 face 把 payment audit `sequence_id` 声明为参与列仍不等于 signed callback 已消费该不变式；将该行移到更晚合法 audit 之后曾形成 callback/state/audit `200/409/409`。signed callback 现逐行检查目标 payment audit 相对于同 sandbox 全部 audit 的 `sequence_id`/`created_at` 顺序，`sequence_id` 与 `created_at_anchor` 进入独立三路径损坏格，矩阵扩为 49 单项与 1,176 两两组合。相同复核还发现，跨 standard/seckill 的同 `order_id` 损坏在生产 payment start 入口会让 repository 专用完整性异常逃逸为 500；start 操作边界现按根因翻译为 409，真实公共 API 回归证明不创建 attempt。两项分别再次证明“参与列必须被每条要求一致的路径实际消费”和“完整性异常必须在每个拥有它的操作边界显式翻译”；清单已覆盖这两个复发类，无需新增占位规则。

## Governance — Terminal route, completeness budget, and Elasticsearch hardening scope

- 现象：连续切片 Goal 若只依赖对话中的终点、内部表面对抗预算与本地 Elasticsearch 安全假设，后续 session 无法从仓库判断何时自然结束、哪些完备性异议可由所有者封顶，以及哪些最小权限声明有真实运行时证据。
- 证据链接：2026-07-23 所有者 Level 3 决策，随 CB-113 PR #49 持久化。
...
- 现象：候选索引最初已要求 owner snapshot、Broker journal 与 `SYNC_EVENT` marker 集合相等，但 catch-up 写路径仍使用允许重复 JSON object key 且无采集字节上限的 Elasticsearch decoder。独立复核又在最后一次 marker 校验与 Broker ACK 之间注入一个已被 owner 新版本覆盖的合法旧事件；commit 只比较 owner 版本，因而会 ACK 一个从未进入已验证 marker 集合的事件。多消息 ACK 若首条成功、后条失败，重启后 Broker 只重投后缀，已经 ACK 的前缀又会被候选 marker 对账误判为孤儿。第一版恢复证据把“第二次 ACK 失败”与“封存集合并集”拆成两个测试，删除 coordinator 的封存/重启接线后仍可能同时绿；候选枚举还会把两个相同 `SYNC_EVENT` hit 覆盖成一个 map 成员。随后复核构造双 coordinator 交错：A 读到不含 E 的 PREPARED 控制记录后暂停，B 封存 E 并取得 ACK checkpoint，A 再用无条件整文档 PUT 写入陈旧 SWITCHED 状态，从持久记录中删掉 E；B 仍可凭进程内 marker map ACK，导致重启后已 ACK 前缀无法重建。
- 证据链接：[slice PR #49](https://github.com/ChanTso/citybuddy/pull/49)
- 根因：重复键拒绝与有界读取只落在 rebuild client，遗漏了复用 incremental projection 的 catch-up client；marker 对账承诺停留在进程内 validation 参数，没有在第一个不可逆 ACK 前持久化。Broker 的逐条 ACK 不是事务，失败后的 Broker 可见集合不能重建已经成功 disposition 的前缀。即使增加持久 descriptor，`seal_journal_events`、`prepare_switch` 和 `complete_switch` 仍各自执行无版本条件的 read-modify-write；grow-only 只是调用者意图，不是存储机制保证。按预期结果拆分的单元测试只证明局部性质，没有证明生产 coordinator 把持久封存、并发控制记录更新、失败、进程重建、后缀重投、复验和完成连接成一条路径。
- 解决：所有 Elasticsearch JSON 响应边界共用重复键拒绝 decoder，并在读取时以固定上限加一字节拒绝过大成功/错误响应；候选分类对每种记录都在写入 map 前拒绝重复 document id。rebuild 控制记录在任何 ACK 前持久封存按 event id 排序的精确 journal descriptor 集合；读取保留 `_seq_no`/`_primary_term`，创建使用 `op_type=create`，所有更新使用版本 CAS，冲突后有界重读最新记录并重新判定。`journalEvents` 在每次写入前执行不可变子集校验。封存返回同一个不可变规范 `JournalCheckpoint`（完整 descriptor tuple 及其确定性 marker map），该值同时传给 Broker commit 与随后的 complete；complete 对 PREPARED 和已 SWITCHED 都按 event id/全内容验证 expected checkpoint 包含关系并保留最新超集。commit 在 ACK 前一次性预检全部当前 pending：每条 owner-covered event 必须与已封存 marker 全列一致，验证后到达的旧事件固定失败并保持未确认。端到端恢复测试由 coordinator 用生产封存实现写入两事件检查点，第二 ACK 确定性失败后构造只保留持久控制记录与候选 marker 的新 client、新 journal 只收到后缀，重新验证完整两成员集合后才 ACK 与完成；真实 Elasticsearch 双 coordinator 测试又强制 A 陈旧完成写与 B 新封存交错，证明 A 的 CAS 冲突、重读、保留超集以及 ACK 后新进程重建。真实运行时权限证据同时证明 `knowledge_indexer` MySQL 账户不存在、commerce/cs 两库拒绝，以及 Support Redis 无凭据、跨前缀读取和管理命令拒绝。
- 结论：非原子外部 receipt disposition 必须在首个 ACK 前持久承诺完整已验证集合；否则 validation-to-ACK 新到达事件和部分 ACK 前缀都会逃出集合对账。可恢复性不能只依赖仍可重投的后缀，已经成功 ACK 的成员也必须由本地持久检查点重建。grow-only 不变式若没有存储层版本 CAS，只是会被陈旧整文档覆盖打破的愿望；创建、封存、准备和完成必须统一使用 create-only/CAS，冲突后必须重读重判，不能重交陈旧文档。ACK 与 completion 必须共享包含完整 descriptor 的同一个不可变 checkpoint，而不是各自派生 digest 或进程内 marker map。共享调用链上的解析边界必须逐个核对，修复一个 client 不等于所有 Elasticsearch 响应都拒绝重复键或在采集前有界。分开的局部性质测试不能替代跨重启生产接线证据；删除关键接线仍能全绿就是证据假绿。
...
- 解决：应用容器保持为当前进程直接拥有的 foreground child，使用 exact Compose network、内核分配的宿主端口和 exact label/PID cleanup；manifest 明确记录真实容器拓扑与无显式资源限制。删除不准确 bundle并完整重跑。随后只重命名非语义 control intent 变量、autosquash 未发布 branch history，并在正式 run 前要求 repository Gitleaks 对全部可达 commits `no leaks found`；没有增加 allowlist、exclude 或关闭规则。虽然 targeted tests、canonical zero-rewrite、checksum/reconstruction 与完整 CI 均通过，最终独立复核仍构造出 classifier/checker、Q04/Q07/Q09、cleanup inventory、nested schema、record count 与 acquisition-bound false green。两轮恢复预算已耗尽，结果 bundle被删除，切片标记 `BLOCKED`，不发布性能 claim。
- 结论：可重建数字与绿灯测试都不等于完整证据；manifest 对执行拓扑或参数的错误描述、checker 未拒绝的矛盾样本、以及只检查 working tree/staged diff 的 policy scan 都能制造 false green。正式 evidence 必须绑定同时通过运行时事实核对、对抗性 reconstruction 和 committed-history policy scan 的 candidate；预算耗尽后必须撤销 claim并阻塞，而不是用第三轮修正规避治理上限。
