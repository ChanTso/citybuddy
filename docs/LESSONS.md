...
- 根因：回放路径在解析已落盘 callback 真值之前进入了可变 sandbox 存活和加锁路径；`DataAccessException -> 503` 又同时覆盖连接不可用、`CannotAcquireLockException` 与 `DuplicateKeyException`，使瞬时竞争和约束冲突无法按根因收敛。测试编排则把 MySQL 容器恢复或管理连接可用误当作应用连接池已恢复，并一度用会改变 Docker 动态宿主端口的 stop/start 制造了无法重连的假恢复窗口。
- 解决：callback 入口先在独立只读事务中用 callback key/event 稳定身份定位已提交记录，再对 callback、attempt、order、ledger 与 evaluation audit 的完整承诺逐列复核；相同意图直接返回既有结果，冲突或数据损坏固定 409，且不再读取当前 sandbox liveness。仅无已提交结果的请求进入加锁路径；1213 deadlock、1205 lock timeout 与 duplicate-key 竞争都在 service 内重新解析提交真值，收敛为既有结果或 409。HTTP 503 映射收窄到连接/资源不可用和事务无法建立。真实集成以 MySQL `offline_mode` 在不改变 endpoint 的情况下断开运行连接，先证明断连固定 503，再通过触达数据库的认证 `/api/eval/state` 200 门确认 Hikari 重连；门后五次不同 callback key 回放均固定 409，相同意图回放返回既有结果，已死亡 sandbox 的已提交回放仍返回 200 且无二次写入。
