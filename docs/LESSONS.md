...
- 解决：全部集成脚本统一使用共享端口租约器，只在 20000–31999 分配；每个候选先做真实 loopback bind 预探测，再以 hard-link 原子发布已经完整写入的租约文件，并在租约持有后复探一次。租约同时记录 PID 与进程启动指纹，缺失、空白、损坏、旧目录或 PID 已复用的记录均可回收；冲突时最多顺延 4096 个候选。本地服务清理必须 `kill` 后 `wait`，容器停止后才释放租约。行为回归真实预占首选端口，并让知识搜索与 evaluation identity 两个完整套件同时从同一候选开始；两者均成功且最终无残余租约或 Docker 资源。
